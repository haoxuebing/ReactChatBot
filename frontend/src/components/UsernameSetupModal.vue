<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-sm bg-white rounded-2xl shadow-xl p-6">
      ...
      <button
        @click="handleConfirm"
        class="w-full mt-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-medium transition-colors"
      >
        开始使用
      </button>
    </div>
...
import { DEFAULT_USERNAME, saveUsername } from '../utils/userStorage'

const emit = defineEmits(['confirm'])

const inputValue = ref(DEFAULT_USERNAME)
